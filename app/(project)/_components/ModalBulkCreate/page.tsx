...
interface ModalBulkCreateProps {
    show: boolean,
    hideBulkModal: () => void,
    storyStatus: UserStoryValue,
    storyStatuses: Array<UserStoryValue>
}

const ModalBulkCreate = ({ show, hideBulkModal, storyStatus, storyStatuses }: ModalBulkCreateProps) => {
    const [popupShown, setPopupShown] = useState(false);
    const [curStatus, setCurStatus] = useState<UserStoryValue>(storyStatus);

    const handleChangeStatus = (status: UserStoryValue) => {
        setCurStatus(status);
        setPopupShown(false);
    }
        ...
        <div className={`items-center justify-center transition-opacity duration-300 bg-white opacity-95 fixed top-0 left-0 right-0 bottom-0 z-[98] ${show ? `flex` : `hidden`} `}>
            ...
            <form className="basis-[600px] grow-0 max-w-[600px]">
                ...
                <fieldset className="mb-4 relative w-full">
                    <textarea
                        cols={200}
                        className="max-h-[12rem] min-h-[15rem] bg-white border-2 border-[#d8dee9] rounded-[3px] text-[#4c566a] m-0 w-full pr-4 pl-[15.2px] py-[4.8px]"
